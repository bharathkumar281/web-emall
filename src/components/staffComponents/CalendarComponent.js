...
import { Card, Row, Col, FormControl, Form, Table, Container } from 'react-bootstrap';
...
class BookingCalendar extends React.Component {
    ...
    handleClick(dateObj) {
        var s = dateObj, e = dateObj;
        if (this.state.startDate) {
            let c1 = MyDate.compare(this.state.startDate, dateObj);
            let c2 = MyDate.compare(this.state.endDate, dateObj);

            if ((c1 * c2) !== 0) {
                if (c2 > 0) e = this.state.endDate;
                else s = this.state.startDate;
            }
        }
        let list = MyDate.range(s, e, 'iso').reverse();
